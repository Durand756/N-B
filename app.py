...
import time
import importlib
import importlib.util  # Import manquant
import glob
import sys
...
def load_commands():
    """Charger dynamiquement toutes les commandes depuis le dossier Commandes/ ou commandes/"""
    global COMMANDS
    COMMANDS = {}
    
    # Chercher le dossier de commandes (majuscule ou minuscule)
    commands_dir = None
    ...
    if not commands_dir:
        logger.error(f"❌ Aucun dossier de commandes trouvé dans {script_dir}!")
        logger.error(f"🔍 Cherché: {', '.join(possible_dirs)}")
        return
    
    # Ajouter le répertoire parent au sys.path si nécessaire
    ...
    # Filtrer les fichiers __init__.py
    ...
    for file_path in command_files:
        try:
            ...
            module_name = f"commands_{command_name}_{int(time.time())}"
    ...
    logger.info(f"📊 {len(COMMANDS)} commandes chargées au total: {list(COMMANDS.keys())}")
